  ...
  def writeCompileAndTestConfiguration(mojo: BloopMojo, session: MavenSession, log: Log): Unit = {
    ...
    def writeConfig(
      ...
      val config = {
        ...
        val project = Config.Project(name, baseDirectory, Some(root.toPath), sourceDirs, dependencyNames, classpath, List.empty, out, classesDir, resources, `scala`, java, sbt, test, platform, resolution)
