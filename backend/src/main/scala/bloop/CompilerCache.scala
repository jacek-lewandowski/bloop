...
final class CompilerCache(
  ...
  private def newCompilers(key: CacheKey): Compilers = {
    ...
    val javaCompiler =
      if (isSystemJava(key.javaInstance.jdkHome))
        ...
        getForkedJavaCompiler(key.javaInstance.jdkHome.toString).getOrElse(getSystemJavaCompiler)
  ...
  private def isSystemJava(javaHome: AbsolutePath): Boolean =
    Files.isSameFile(javaHome.toFile.toPath, JdkInstance.Default.jdkHome.toFile.toPath)
